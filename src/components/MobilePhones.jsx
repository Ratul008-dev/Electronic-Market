...
const MobilePhones = () => {
  ...
  useEffect(() => {
    fetch('./Phones.json')
      .then(res => res.json())
      .then(data => setmobilePhonesdata(data))
  ...
  return (
    <div className="MobilePhones-list">
      {mobilePhonesdata.map((item) => (
        <div className="MobilePhones-card" key={item.id}>
          <div className="MobilePhones-image">
            <img
              src={item.image1}
              alt={item.name}
          ...
          <div className="MobilePhones-info">
            <h3>{item.name}</h3>
